...
class DBWriter( DBAccess ):
    ...
    def write_to_db( self, sim, sim_name, write_meta_models = True, write_models = True  ):
        """
        ...
        for model_name, model in self.sim.edit.models.items():
            if ( write_models == True ):
                ...
                self._write_model_to_db( model_name, model )
            else:
                # Retrieve model from the database (stored as simulation tool).
                self._retrieve_model_from_db( model_name )
        ...
        for node_name in self.sim.edit.nodes.index:
            ...
            self._write_node_to_db( node_name, node, sim_id )
    ...
    def _write_model_to_db( self, model_name, model ):
        ...
        if model[ 'command' ] is not None:
            ...
            self.current_session.query( insert_command ).one()
        ...
        if model[ 'files' ] is not None:
            ...
            self.current_session.query( insert_files ).one()

        # Collect model ID.
        self.model_ids[ model_name ] = model_id


    def _retrieve_model_from_db( self, model_name ):
        model = self.current_session.query( SimulationTool ).filter_by( name = model_name ).one()
    ...
    def _write_node_to_db( self, node_name, node, sim_id ):
        ...
        insert_is_first = func_insert_string_parameter_node( node_id,
            'is_first', str( node[ 'is_first' ] ) )
        self.current_session.query( insert_is_first ).one()

        self._write_node_init_vals_to_db( node_id, node[ 'init_values' ] )
